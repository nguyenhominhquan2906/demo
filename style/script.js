document.addEventListener('DOMContentLoaded', () => {
    ...
    function checkGateAnswer() {
        ...
        if (answer === '12a6') {
            gateInput.classList.add('correct');
            ...
            setTimeout(() => {
                ...
                gateInput.classList.remove('wrong');
                if (card) card.classList.remove('blur-all');
            }, 2000);
        }
    }

    if (gatekeeper && gateAlert && gateSubmit) {
        gateSubmit.addEventListener('click', checkGateAnswer);
